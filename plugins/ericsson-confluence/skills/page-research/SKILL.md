...
When the page is known, use <tool name="confluence_get_page" mode="read">confluence_get_page</tool> with its numeric <code>content_id</code>. Then use <tool name="confluence_get_page_body" mode="read">confluence_get_page_body</tool> with the same <code>content_id</code> and <tool name="confluence_list_comments" mode="read">confluence_list_comments</tool> with that <code>content_id</code> and a bounded <code>max_results</code>. This is the default page, body, comments sequence.
