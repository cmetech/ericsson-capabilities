...
NODE_KEYS = {"id", "kind", "prompt", "command", "message", "depends_on", "when",
             "output", "side_effects", "toolset", "tools", "allowed_tools"}
...
def validate_workflow(doc: dict):
    ...
    if isinstance(requires, list):
        if any(not isinstance(value, str) or not value for value in requires):
            errors.append("requires must be a list of non-empty service ids")
    elif requires and not isinstance(requires, dict):
        errors.append("requires must be a list of service ids or a mapping")
    ...
    for i, node in enumerate(nodes):
        ...
        tools = node.get("allowed_tools", node.get("tools"))
        if "allowed_tools" in node and "tools" in node:
            errors.append(f"node {nid}: declare allowed_tools or tools, not both")
        if kind == "tool" and tools is not None and (
            ...
            errors.append(f"node {nid}: allowed tools must be a non-empty string list")
        if kind != "tool" and tools is not None:
            errors.append(f"node {nid}: only kind tool may declare allowed tools")
