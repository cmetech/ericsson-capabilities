...
Natural-language guidance and the packaged workflow still own ticket-to-change
reasoning, review, sequencing, and approvals. Direct leaves such as
`<brand> gitlab branch create-ticket group/project ERIC-123 --summary 'repair login' --dry-run`
and `<brand> jira issue comment ERIC-123 --body-file summary.md --dry-run`
can support supervised component work, but they do not recreate the orchestration.
Their availability does not change this flow's status; each standalone connector
must still be enabled and configured.
