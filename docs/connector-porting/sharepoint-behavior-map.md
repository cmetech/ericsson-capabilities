...
|---|---|---|---|
| E-01 | `RESULT` / combined JSON | Produces one combined object with status, error, times, counts, skipped sites, owned sites, and nested per-site metadata/users/permissions/members/lists/subsites. | **Preserve normalized structure** as bounded tool data and optional structured artifact in Task 7. Avoid process-global mutable run state. |
